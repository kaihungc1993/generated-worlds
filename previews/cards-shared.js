// Shared engine for the card-game previews: data loading, card DOM,
// hover tilt + holo shine, and the three.js summon / dive stages.
...
export const CATS = {
  ...
  sim: { label: 'Sim Environments', short: 'SIM', color: '#ffb86b', kind: 'scene' },
  worlds: { label: 'Generated Worlds', short: 'WORLD', color: '#7c9eff', kind: 'scene' },
  game: { label: 'Playable Game', short: 'GAME', color: '#8ef0c0', kind: 'game' },
};

const ASSET_SLUGS = [
  'dishwasher', 'office-chair', 'plastic-parts-bin-drawer-cabinet',
  'clamp-or-vise', 'mechanical-hinge-assembly', 'machine-enclosure-door',
  'wireless-earbuds-charging-case',
];
const SIM_SLUGS = [
  'theme-park', 'water-park', 'theme-park-ride-system', 'bathhouse',
  'supermarket-interior', 'shopping-mall-interior',
  'robotic-arm-assembly-cell', 'dockyard',
];
...
/** Fetch manifests and build the card list (assets, sim, worlds, game teaser).
 * Pass { all: true } for every object + every scenes-collection entry. */
export async function loadCards({ all = false } = {}) {
  const [blender, evals] = await Promise.all([
    fetch('/data/blender.json').then((r) => r.json()),
    fetch('/data/evals.json').then((r) => r.json()),
  ]);
  const bySlug = Object.fromEntries(blender.map((e) => [e.slug, e]));
  const cards = [];

  const assetSlugs = all
    ? blender.filter((e) => e.group === 'objects').map((e) => e.slug)
    : ASSET_SLUGS;
  const simSlugs = all
    ? blender.filter((e) => e.collection === 'scenes').map((e) => e.slug)
    : SIM_SLUGS;
  ...
  for (const slug of assetSlugs) {
    ...
    if (!e) continue;
    cards.push({
      id: slug, cat: 'assets', kind: 'asset',
      title: e.title, desc: shortDesc(e.prompt),
      thumb: '/' + e.thumbnail, glb: '/' + e.url, sky: null,
  ...
  cards.push({
    ...
    desc: 'A complete cyberpunk arcade world built in Godot 4 — playable in the browser. Coming to the deck soon.',
    thumb: '/play/ghost/promo/title.webp', glb: null, sky: null,
  ...
  el.innerHTML = `
    <div class="card-tilt">
      <div class="card-inner">
        ...
        <div class="face back"><div class="sigil">${SIGILS[card.kind]}</div></div>
      </div>
    </div>`;
  return el;
}
...
function waitTransition(el, timeout = 900) {
...
/** Yu-Gi-Oh summon flow for an asset card. Resolves when dismissed. */
export async function summonFlow(stage, fx, card, el) {
  ...
  // fly to center, grow, spin-flip, charge glow
  ...
  clone.style.transform =
    `translate(${innerWidth - 128 - (rect.left + rect.width / 2)}px, ${innerHeight * 0.56 - (rect.top + rect.height / 2)}px) scale(0.6) rotate(4deg)`;
  showCaption(fx.caption, card);

  await onDismiss(fx, 'dismiss ✕');
  // hard cut straight back to the fan — no reverse-fly, no fades
...
/** Scene dive flow for a sim/world card. Resolves when dismissed. */
export async function diveFlow(stage, fx, card, el) {
  const { clone, rect } = makeClone(el);
  el.style.visibility = 'hidden';
  fx.dim.classList.add('on');
  fx.help?.style.setProperty('opacity', '0');
  // measure the artwork region while the clone is untransformed, so the
  // final zoom can target the art rectangle rather than the whole card
  ...
  clone.classList.add('charging');
  const mat = addMatLabel(clone, 'MATERIALIZING WORLD…');
  ...
  // zoom INTO the artwork: scale + translate so the art rect covers the
  // viewport while the text portion slides off-screen below
  await sleep(750);
  ...
  // teleport moment: white flash masks the cut from art to live world
  raysBurst(fx.rays);
  stage.show(false);
  pill.remove();
  fx.fill.classList.remove('on');
  ...
  await onDismiss(fx, '← back to deck');
...
// ---------------------------------------------------------------- side rail (focus views)

/** Compact category-grouped card rail shown during focus views.
 * Returns { el, show(currentId), hide(), setCurrent(id) }. */
export function makeRail(cards, onPick) {
  const el = document.createElement('div');
  el.id = 'rail';
  el.classList.add('off');
  const groups = Object.keys(CATS).map((catId) => {
    const items = cards.filter((c) => c.cat === catId);
    if (!items.length) return '';
    const cat = CATS[catId];
    return `<div class="rail-cat" style="--accent:${cat.color}">${cat.label}<i>${items.length}</i></div>`
      + items.map((c) => `
        <button class="rail-item" data-id="${c.id}" style="--accent:${cat.color}">
          <img src="${c.thumb}" alt="" loading="lazy" />
          <span>${c.title}</span>
        </button>`).join('');
  }).join('');
  el.innerHTML = `
    <button class="rail-toggle" title="toggle deck">⟩</button>
    <div class="rail-panel">
      <div class="rail-head">DECK</div>
      <div class="rail-scroll">${groups}</div>
    </div>`;
  document.body.appendChild(el);

  const toggle = el.querySelector('.rail-toggle');
  toggle.addEventListener('click', () => {
    const collapsed = el.classList.toggle('collapsed');
    toggle.textContent = collapsed ? '⟨' : '⟩';
  });
  el.querySelector('.rail-scroll').addEventListener('click', (e) => {
  ...
  return {
    ...
    setCurrent(id) {
      el.querySelectorAll('.rail-item').forEach((b) => b.classList.toggle('current', b.dataset.id === id));
      el.querySelector(`.rail-item[data-id="${id}"]`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
    },
  };
}
 ...
 * Unlike summonFlow it does NOT own dismissal — the page keeps focus running. */
...
/** Game teaser flow: card flies up, poster reveal, no game integration. */
